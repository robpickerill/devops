...
* TCP Connection ID
  ...
  The unique ID only holds if few things hold. First of all we need to
  make sure Host A the initiator of the connection picks up a unique
  source port ID. It should not accidentally chose the same source
  port number, which it already using in some other connection to the
  same service on Host B.
