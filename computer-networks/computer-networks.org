...
*** First step
    ...
    to the beginning of its stream. We do this rather than just say
    assume zero, for a bunch of reasons. Its very helpful to randomize
    starting sequence number for security reasons.
...
*** Second step
    ...
    going to synchronize you, with my starting sequence number. But it
    will also set the ACK bit which means that the acknowledgement
    number in the packet is valid. Node acknowledges not the last byte
    received, but rather the first byte that has not been received.
