...
** TCP Connection - FSM
   ...
   at the server, this leads to a transition. The event is receiving a
   SYN message. The action is to send a SYN/ACK message in
...
* Reliable Communications - TCP Header
  Standard TCP header is 20 bytes long. So there are five rows of four
  octets each.
  ...
  - The next two fields are the TCP sequence number. So this denote
    ...
    acknowledges the last byte received plus one. And so if we were to
    send the segment 4000 to 4999 and the other side received it, it
    would have send an acknowledgement number of 4500. That is in TCP,
    the ACK is not for the last byte received, but that plus one.

    TCP ACK packets are TCP segments that have no data. All they are
    doing is counting the acknowledgement numbers for it. This happens
    when traffic is unidirectional, sending lots of data in one
    direction, but there is no data coming back. If the flow is
    bidirectional then these acknowledgement numbers are just going to
    be incorporated into the data segment as they are being sent.
  
  - The next field is =checksum=, which is computed over the TCP
    pseudo header, which is the - TCP header as well as some of the IP
    header. So, the checksum covers the pseudo header, the TCP header,
    and then the data within the TCP segment. The checksum in some way
    stretches before the packet to the pseudo header filled in from
    the IP header and stretches to the end of the segment. Checksum is
    simple one's compliment.
  ...
  - Control bits: U, A, P, R, S and F
    + U: Urgent bit - that means this data is particularly urgent. So
      it gets to the application quickly.
    + P: Push bit - push the data to the application.
    + ACK bit - is set to one, if the acknowledgement number field is
      ...
      for the first one that initiates the connection. Because when we
      initiate a connection, we don't know what the other side's
    ...
    + SYN bit - it says this is the starting number please synchronize
      to this number. So when we first open a connection we sent a
      packet with the ACK bit not set but with the =SYN bit= set and
      then the sequence number. The other side can then respond and
      say alright I am going to acknowledge that sequence number and
      send you one of mine. In this case both of these fields are
      valid. To which then initial sender will respond and say okay I
      acknowledge your sequence number.
