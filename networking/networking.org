...
** IP address and port
   ...
   addresses. One is an IP address that is used by the network layer
   to deliver the packet to the end host. Second is port, that is used
   by the transport layer to deliver the data to the correct
   application.

   For example, web server usually runs on port 80. So when we send a
