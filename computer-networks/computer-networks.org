...
*** Packet switch features
    ...
    - They also have *buffers*. If packets arrive at the same time and
      ...
      packets during those times of congestion.
...
** End to End Delay
   ...
   to B, from when the first bit is sent, until the last bit arrives.
   ...
   means switch S1 is gonna wait until the whole packet arrives, until
   it looks up the address and decides where to send it next. So the
   end to end delay from A to B would be sum of individual end to end
   delay
   #+BEGIN_EXAMPLE
