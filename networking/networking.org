...
*** VPN
    ...
    encapsulated inside TCP segment, and then IP packet. Then it is
    encapsulated inside TLS segment, which protects the message. Then
    this packet is encapsulated inside TCP segment, inside IP packet
    and then ethernet frame.
