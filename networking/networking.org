...
* Internet 4-layer model
  ...
  they also need to be confident that data is delivered correctly,
  with any lost or corrupted data automatically retransmitted until is
  received correctly.
  ...
  the operations that make up the internet, so that we can reuse these
  instead of creating them again for each and every application.

** Link layer
   Ethernet and wifi are two example of link layer. Its job is to
   carry packets by link by link, one link at a time.

** Network layer
   Network layer's job is to deliver packet end to end, across the
