...
** Stop and Wait
   Stop and wait algorithm is very simple. It has at most one packet
   in flight at any time from the sender to the receiver.
   ...
   packet. So there is a timeout, at which point it tries
   again. That's the basic algorithm.

*** Receiver FSM
    The receiver has one state - finite state machine, which is wait
    for packet. When it receives new data, it sends an
    acknowledgement, or when it receives data it sends an
    acknowledgement for that data. And if the data is new, it delivers
    that data to the application.
...
*** Sender FSM
    ...
    is when there's a timeout. So, this is the case where it has sent
    a packet of data but it hasn't received any acknowledgement. It's
    waiting and then time's up. So then it just tries re-sending. So
    it want's to pick this time out so that it's conservative. It's
    pretty sure that the data or the subsequent acknowledgement has
    been lost. So, it has only one packet in the network at any time.
...
**** Data is lost
     Second case, data is lost. The sender sends data, it's lost in
     the network and so the sender times out, and tries re-sending the
...
**** Duplicate Packet
     ...
     another data packet. But let's say that, in fact, this data
     packet is lost. So now first re-transmission of the first data
     packet arrives at the receiver and the receiver acknowledges
     it. The sender does not know whether this acknowledgement here,
     is for the re-transmission of the data or it's for the new data
     packet. And so here we can have error, where if it assumes it was
     for the re-transmission of the old data somebody has to keep
     track of that.
     ...
     detect *duplicates*. How do we know when acknowledgements are
     from re-transmissions or duplicated copies of packets versus new
     data.
     ...
     one, data zero, ACK zero. Now the receiver can tell if this is
     new data or a duplicate.
...
*** Major problem with stop and wait
    ...
    at 10Mb/s. The round trip time is 50 milliseconds, that means A
    can send one packet to B, if that packet is received successfully
    50 milliseconds later, we will get an acknowledgement. Let's
    assume we are sending Ethernet frames, so the size of data is
    basically 1.5 kilobytes or 12 kilobits. 1000 milliseconds divided
    by 50, means we can send 20 packets per second. Now 20 packets per
    second times 12 kilobits is equal to 240 kilobits per
    second. Assuming no packets are lost, just a constant RTT 50
    milliseconds. But the bottle neck is 10Mb/s, so this means that
    this stop and wait protocol is using two percent of what the
    communication could be. So stop and wait works but it can be
    astonishingly inefficient. Node A could be sending data much
    faster than what stop and wait allows. Solution is to use sliding
    window protocol.
...
** Sliding Window
   ...
   bottleneck of 10Mb/s, let's say we are sending ethernet frames at
   10 kilobits per packet. And we have 20 round trip times, that means
   the sliding window is gonna be 10Mb/s divided by twenty round trip
   times, which is equal to 500 kilobits per round trip time. So we
   are looking at a sliding window of around 41 packets, that would be
   492 kilobits. So if we had a sliding window of 40 packets, then we
   would be able to sustain a ten megabit connection from San
   Francisco to Boston with a round trip time of 50 milliseconds.
   
   Let's say we have a sliding window of size 3. Sender will send
   three packets. Let's call them D0, D1, D2 and the receiver can then
   acknowledge them. Ack zero, ack one and ack two. As soon as the
   acknowledgement zero arrives the sender can then send data three,
   as soon as the acknowledgement one arrives, the sender can then
   send data four, as soon as the acknowledgement two arrives the
   sender can then send data five. This is the basic idea, rather than
   having one packet we could have many packets.
   ...
   equal to the send window size. So this means that if it has
   received packet N, a packet with a sequence number of n, the sender
   can not send packet past (n + SWS).

   Let's say sending window size is 5, and the last acknowledgement
   received is 11. This means that the sender cannot send a packet
   past 12, 13, 14, 15 and 16. It is not allowed to send seventeen
   until it gets acknowledgement for 12. When we acknowledgement we
   advance LAR as necessary and we buffer upto the sending window size
   segments, in case suddenly we get an acknowledgement and then we
   want to send whole bunch of new data.
   ...
   segment minus that last thing we received must be less than or
   equal to the receive window size. So if we have a "receive window"
   size equal to 5 and a last segment received equal to 3, then its
   not going to accept anything past 4, 5, 6, 7 and 8. So if it
   receives suddenly segment 10, it won't accept it and it will drop
   it. Now if the receive packet is less than this acceptable packet,
...
*** Go-back-N
    ...
    that's larger than one. Well in this case we are gonna need
    sending window size plus one sequence number. Well say send window
    size is equal to three, so the sender sends zero, one, and two,
    and lets say those are all acknowledged, and so the receiver
    acknowledges a zero, and acknowledges one, and acknowledges
    two. Well when it acknowledges zero, the senders gonna send three,
    slide the window forward, when it acknowledges one, its gonna send
    four, and when it acknowledges two, it's gonna send five. So lets
    say that three is dropped. The receiver is still going to receive
    four and five, so it can ack two, its going to send ack two, ack
    two. The sender is going to timeout and re-send three. This is
    called Go-Back-N protocol. Because, the receive window size was
    one, the receiver can not buffer four or five. When a single
    packet is lost in this case three, the sender has to go back and
    has to re-transmit the entire send window worth of packets. It has
    to re-transmit three, four and five. In contrast if the receive
    window size have been three then the receiver could have buffered
    four and five. The sender would only have had to re-transmit
    three, then we get an ack five, and it could go on and send six,
    seven and eight.
*** Flow Control
...
*** Example
    ...
    advance the window and send five. Now lets say packet three
    arrived successfully and is acknowledged, but packet four is lost
    in the network.

    So now we have this case where ack three has been sent, packet
    four is lost, then packet five arrives at the receiver. Now the
    receiver is going to send another acknowledgement three, again
    because of cumulative acknowledgements, and now sender heard ack
    three and then another ack three, waits times out and re-sends
    four. So it will resend four and lets say four arrives. Now this
    receiver can acknowledge four so it can ack four, but because its
    received window size was two, it actually had five buffered, and
    so it can acknowledge five, and will send ack five.
...
*** Window size
    ...
    basically their sum, RWS plus SWS sequence number. We need
    sequence number spaces at least as big as the sum of the window
    sizes.
...
** Design approach
   So when we are implementing transmit protocols, the only thing we
   ...
   - one packet was lost - retransmit entire window with the go-back-n
     policy.
   - one packet was lost - retransmit one packet, wait for one round
     trip time, and see if we get acknowledgement and see where the
     acknowledgement puts and then just do selective repeat.
