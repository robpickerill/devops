...
* Error Detection
  Networks are not perfect and neither to hosts that run on them, they
  can introduce errors. We need to be able to detect these errors.
...
** Checksum
   ...
   error detection guarantee. It can detect random errors but its easy
   to fool a checksum with as few as 2 bit errors, if the two bit
   errors cancel each other out. For example if one bit error adds 32
   and another bit error subtracts 32, then a checksum won't catch the
   error.
