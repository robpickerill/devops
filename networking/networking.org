...
* Packet Switching
  A network switch does following job:
  1. Forwards the packet to its outgoing link.
  2. Share the resources efficiently.
  3. No per flow state.

  - Flow :: A collection of datagrams belonging to the same end-to-end
            communication.
