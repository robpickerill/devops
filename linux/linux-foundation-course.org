...
** root account
   =root= is the most privileged account in Linux system. This account
   has the ability to carry out all facets of system administration,
   including root accounts, changing passwords, examining log files,
   installing software, etc. It has no security restrictions imposed
   upon it.
