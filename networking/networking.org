...
*** Checksum
    ...
    UDP checksum calculation also includes the portion of IPv4
    header. It includes the source ip address, destination ip address
    and the protocol id. This violate the principle of layering. But
    the rationale is, it allows UDP layer to detect datagrams that
    were delivered to the wrong location.
