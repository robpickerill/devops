...
** End to End Delay
   ...
   packetization delay and propagation delay.

   #+BEGIN_EXAMPLE
   end-to-end-delay-for-first-link = p/r + l/c
   #+END_EXAMPLE
   ...
   #+BEGIN_EXAMPLE
   end-to-end-delay = sum(p/r_i + l_i/c)
   #+END_EXAMPLE
   ...
   all of the packets are gonna have to fight or contend for that
   outgoing link.

   Packet buffer helps prevent us from having to drop any packets. The
   bigger the buffer is the less likely we are to have to drop a
   packet, that wants to travel across the link. These packet buffers
   are going to be in all of the switches. Every packet switch has
   buffers, and they are fundamental to packet switching. If we did
   not have packet buffers, then we would lose a packet every time two
   packets showed up at the same time wanting to travel across a link.

   We also need to incorporate congestion delay in our overall end to
   end delay. =Q(t)= is the queuing delay of a packet in the queue due
   to congestion. Some people call queue as packet buffers but
   generally these are first come first serve queues.
