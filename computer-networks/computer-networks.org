...
* World Wide Web(HTTP)
  ...
  commands such as *GET*, *PUT*, *DELETE*, *INFO*, as well as other
  responses such as 400 Bad Request. HTTP is all in ASCII text: for
  example, the beginning of a GET request looks like:
  #+BEGIN_EXAMPLE
  GET / HTTP/1.1
  #+END_EXAMPLE
...
** Connection Setup
   ...
   First of all host A, sends a message to host B indicating that the
   TCP layer at A wants to establish a connection with the TCP layer
   at B. This message is called "SYN" message which is short for
   synchronize, because A also sends a base number it will use to
   identify bytes in the byte stream. If A sends "0", then the number
   will start at 0, if it sends "1000", then the number will start
   at 1000.
...
*** Acknowledgement sequence number
    ...
    sequence numbers for both the directions in a segment. This way
    TCP piggybacks acknowledgement on the data segments travelling in
    the other direction.
...
** HTTP Request
   ...
   If we want to be completely explicit we should specify what happens
   on each state for every event. But this can lead to complicated
   FSMs which have tons of edges. So often instead we specify only
   common cases for the ease of understanding, and have some
   supporting text about other transitions. Or, in some cases, it can
   even be acceptable to leave something undefined. The Internet
   Engineering Task Force, for example, often doesn't completely
   specify every FSM. The idea is that by specifying only the parts
   that are necessary for interoperability, we can leave the
   specification flexible for future exploration. As people use the
   protocol, they will figure out if something is important and can
   specify that extra part later.

** TCP Connection - FSM
   The FSM has four parts, which we can look at separately. The top
   four states are what describes how we open a TCP connection. The
   center state, "ESTABLISHED" is when TCP is sending and receiving
   data. It's after the connection has been established but before
   it's been closed. The next 6 states describe how connections
   close. The state at the bottom, "CLOSED", denotes the connection
   has closed and the node can forget about it.

   [[file://home/yogesh/work/projects/linux-system-administration/computer-networks/Tcp_state_diagram_fixed_new.svg][tcp-state-diagram]]

   The diagram describes how TCP behaves on both sides of the TCP
   three-way handshake. A passive opener is a server. It listens for
   requests for connections from active openers, clients. So when a
   program calls listen(), the socket transitions from the closed
   state to the listen state. The protocol takes no action when this
   happens -- it doesn't send any messages. If the server calls close
   on the socket when it's in the listen state, it transitions
   immediately to the closed state.

   First step of handshake connection is when a client tries to open a
   connection and sends a SYN packet to the server. First transition
   for the client side of the connection is from *CLOSED* to the *SYN
   SENT* state. This happens when the client program calls *connect --
   the event*, and the client *sends a SYN message*.
   
   So once the first SYN is sent, the client is in the *SYN SENT*
   state and the server is in the *LISTEN* state. When the SYN arrives
   at the server, this leads to a transition. The event is *receiving
   a SYN message*. The action is to *send a SYN/ACK* message in
   response. Now the server is in the *SYN RECEIVED* state.

   Client was in the *SYN SENT* state. When it receives the SYN/ACK
   from the server, it transitions to the *ESTABLISHED* state. Its
   action is to send an ACK message, the third message of the SYN,
   SYN/ACK, ACK handshake. Now the client can start sending data to
   the server.

   Now the server which was in the *SYN RECEIVED* state, when it
   receives the ACK from the client, it transitions to
   the *ESTABLISHED* state and can send data to the client.

   So now our sockets are in the ESTABLISHED state. They are
   exchanging data. It's sometimes useful to talk about "tearing down"
   a connection because the word "close" means something in terms of
   system calls. A connection exists after one side "closes" it, as we
   will see.

   There is a symmetry between connection establishment and connection
   tear down. Where connection establishment uses synchronization or
   "SYN" packets, connection tear down uses finish or "FIN"
   packets. If one of the sides of the connection calls close, it
   traverses along to the *FIN WAIT 1* state. The action is to send a
   FIN packet to the other side of the connection. This is called the
   "active closer" because it starts the operation. The other side
   receives the FIN and transit to *CLOSE WAIT* state. It remains in
   this state until the program on its side calls close, at which
   point it sends a FIN.

   A TCP connection is bidirectional. The active closer has closed its
   direction of the connection, so it can't write any more data. But
   it could be, that the passive closer has more data to send. So it
   can continue to send data, which the active closer receives and
   acknowledges. Or it could close its side of connection too. Or it
   could even have decided to close the connection at the same time,
   such that we have two FIN packets crossing each other in the
   network.

   From the *FIN WAIT 1* state, where the active closer is, there are
   three possible outcomes. First, the passive closer might
   acknowledge the FIN but not send a FIN. In this case, the passive
   closer is in the *CLOSE WAIT* state and can continue to send
   data. And the active closer enters the *FIN WAIT 2* state. Second
   the passive closer might close its side too, acknowledging the FIN
   and send a FIN of its own. In this case active closer transits to
   the *TIME_WAIT* state. Finally, it could be that both sides
   actively closed at almost the same time, and sent FINs to each
   other. In this case, both are in the *FIN WAIT 1* state. Each one
   will see a FIN from the other side that doesn't ACK its own FIN. In
   this case we transition to the *CLOSING* state, and when our FIN is
   acknowledged we transition to the *TIME_WAIT* state.

   TCP transition from *FIN WAIT 2* state to *TIME WAIT* state when we
   receive FIN from the other side. It then stays in *TIME WAIT* state
   for a period of time, until it can safely transition to close.

   TCP transition from *LAST ACK* to *CLOSED*, when the passive
   closer's FIN is acknowledged.

* Flow Control
  Flow control provides reliable efficient communication. The basic
  problem flow control tries to solve is when a sender can send data
  faster than the receiver can process it.

** Problem
   Suppose A can send some 500,000 packets per second but the receiver
   B, can only process 200,000 packets per second. This could be
   because B has slower processor, or its network card is not that
   good.

   So if A sends data at this full rate of 500,000 packets per second,
   the 300,000 of those are going to have to be dropped at B. That is
   B will not be able to process them. Only 40% of the packets will
   come through. So this is lot of wasted effort on A's part, its a
   lot of wasted effort in the network and it's also going to
   completely saturate B.
   
** Approach
   The basic approach that flow control takes is to make it so the
   sender doesn't send packets faster than the receiver can process
   them.

   The way this works is the receiver gives the sender some kind of
   feedback, whether its implicit feedback or explicit feedback,
   whether its to slow down or speed up or to set a rate.

* Reliable Communications - Stop and Wait
  Stop and wait algorithm is very simple. It has at most one packet in
  flight at any time from the sender to the receiver.

  So the basic algorithm is sender sends one packet, it then wait for
  acknowledgement from the receiver. When it receives the
  acknowledgement, if it has more data to send then it sends another
  packet.

  If it waits for some time, which is a timeout, and has not heard an
  acknowledgement, then it assumes the packet has been lost. It could
  be that packet was dropped on the router, or it was dropped at the
  receiver or the acknowledgement was dropped. And it re-sends the
  packet. So there is a timeout, at which point it tries again. That's
  the basic algorithm.

** Receiver FSM
   The receiver has one state - finite state machine, which is wait
   for packet. When it receives new data, it sends an acknowledgement,
   or when it receives data it sends an acknowledgement for that
   data. And if the data is new, it delivers that data to the
   application.

** Sender FSM
   The sender finite state machine has two states. The first state,
   it's waiting for data from the application. This is where it's
   ready to send, but the data, the application has not yet provided
   the data to send. When the application calls send, the protocol
   sends a packet with that data, or as much as it can fit in a
   packet. It then enters the wait for ACK state. In this state
   there's are two transitions. The first is if it receives an
   acknowledgement. If the protocol receives an acknowledgement it
   does nothing, it goes back to wait for data. If there is more data
   to send it will send new data. Or if there is no more data to send
   it will wait until the software calls send. The second transition
   is when there's a timeout. So, this is the case where it has sent a
   packet of data but it hasn't received any acknowledgement. It's
   waiting and then time's up. So then it just tries re-sending. So it
   want's to pick this time out so that it's conservative. It's pretty
   sure that the data or the subsequent acknowledgement has been
   lost. So, it has only one packet in the network at any time.
   
** Sample Execution
   There are four sample executions:
*** No loss
    Everything works perfectly. The sender sends its data, the
    receiver receives it, sends an acknowledgement. And now, the
    sender, if it had more data, could send more.
*** Data is lost
    Second case, data is lost. The sender sends data, it's lost in the
    network and so the sender times out, and tries re-sending the
    data. So it is sitting in the waiting for ACK state.
*** Acknowledgement is lost
    Third case is when the data is successfully delivered, but the
    acknowledgement is lost. And so now the sender is in the wait for
    ACK state, it times out and it resends the data. This causes
    receiver to send another acknowledgement, at which point sender
    gets the acknowledgement and continues as in the first case.
*** Duplicate Packet
    Suppose something happens in the network, suddenly a link becomes
    very slow, or there's a big queue somewhere in the network, and
    the acknowledgement is delayed past the time of the timeout. So
    the sender sends some data and receiver acknowledges when it
    comes, but the sender resends the data before the acknowledgement
    arrives. The acknowledgement then arrives very shortly. And now
    the sender knows that the data was acknowledged and it sends
    another data packet. But let's say that, in fact, this data packet
    is lost. So now first re-transmission of the first data packet
    arrives at the receiver and the receiver acknowledges it. The
    sender does not know whether this acknowledgement here, is for the
    re-transmission of the data or it's for the new data packet. And
    so here we can have error, where if it assumes it was for the
    re-transmission of the old data somebody has to keep track of
    that.

    Something the finite state machines has to keep track of. If it
    assumes it's for the new data, that data might not have
    arrived. It could be assuming that data has arrived but it has
    not.

    So, this is a basic problem that comes up in any reliable
    protocol, it comes up in flow control, which is how do we
    detect *duplicates*. How do we know when acknowledgements are from
    re-transmissions or duplicated copies of packets versus new data.

    So, in the case of stop and wait, we can solve the problem with
    a *one bit counter*. And so the idea is that use this one bit
    counter on all data and acknowledgement packets.

    Sender sends data zero, sender receives ACK zero, data one, ACK
    one, data zero, ACK zero. Now the receiver can tell if this is new
    data or a duplicate.

    Single-bit counter makes two simplifying assumptions:
    - First the network is not duplicating packets itself.
    - Second, the packets are not being delayed for multiple
      timeouts. So if packets are not duplicated for more than a
      round trip time, stop and wait works.

** Major problem with stop and wait
   Suppose there is a 10Mb/s link between A and B, and B can receive
   at 10Mb/s. The round trip time is 50 milliseconds, that means A can
   send one packet to B, if that packet is received successfully 50
   milliseconds later, we will get an acknowledgement. Let's assume we
   are sending Ethernet frames, so the size of data is basically 1.5
   kilobytes or 12 kilobits. 1000 milliseconds divided by 50, means we
   can send 20 packets per second. Now 20 packets per second times 12
   kilobits is equal to 240 kilobits per second. Assuming no packets
   are lost, just a constant RTT 50 milliseconds. But the bottle neck
   is 10Mb/s, so this means that this stop and wait protocol is using
   two percent of what the communication could be. So stop and wait
   works but it can be astonishingly inefficient. Node A could be
   sending data much faster than what stop and wait allows. Solution
   is to use sliding window protocol.

* Reliable Communications - Sliding Window
  Sliding window is a generalization of stop and wait. Where stop and
  wait allows only 1 packet in flight at any point in time, a sliding
  window protocol allows up to "N" packets in flight. So, when n is
  equal to one, a sliding window protocol acts like a stop and wait.

  Lets say we have a sliding window protocol with an N equal to 5
  packets. This means node A can have five packets in flight and
  simultaneously there can be five acknowledgements coming back from
  B.

  The idea here is that, if we set N to the correct value then we can
  keep the pipe full. If the link bandwidth is 10Mb/s, then by
  configuring sliding window size, we can have node A send data at a
  rate equal to 10 megabits per second.

  So, in this particular case if we have RTT of 50 milliseconds and a
  bottleneck of 10Mb/s, let's say we are sending ethernet frames at 10
  kilobits per packet. And we have 20 round trip times, that means the
  sliding window is gonna be 10Mb/s divided by twenty round trip
  times, which is equal to 500 kilobits per round trip time. So we are
  looking at a sliding window of around 41 packets, that would be 492
  kilobits. So if we had a sliding window of 40 packets, then we would
  be able to sustain a ten megabit connection from San Francisco to
  Boston with a round trip time of 50 milliseconds.
  
  Let's say we have a sliding window of size 3. Sender will send three
  packets. Let's call them D0, D1, D2 and the receiver can then
  acknowledge them. Ack zero, ack one and ack two. As soon as the
  acknowledgement zero arrives the sender can then send data three, as
  soon as the acknowledgement one arrives, the sender can then send
  data four, as soon as the acknowledgement two arrives the sender can
  then send data five. This is the basic idea, rather than having one
  packet we could have many packets.

  First in the sliding window protocol, every segment has a sequence
  number. So in protocols like TCP this is usually done in terms of
  bytes because they can be of variable size. For simplicity sake we
  will just do in terms of packet numbers. So there is a sequence
  number for every segment. So the sender maintains three variables:
  - size of its sending window
  - last acknowledgement it received from the receiver
  - last segment it sent

  The sender job is to maintain this invariant, that the last segment
  sent minus the last acknowledgement received has to be less than or
  equal to the send window size. So this means that if it has received
  packet N, a packet with a sequence number of n, the sender can not
  send packet past (n + SWS).

  Let's say sending window size is 5, and the last acknowledgement
  received is 11. This means that the sender cannot send a packet past
  12, 13, 14, 15 and 16. It is not allowed to send seventeen until it
  gets acknowledgement for 12. When we acknowledgement we advance LAR
  as necessary and we buffer upto the sending window size segments, in
  case suddenly we get an acknowledgement and then we want to send
  whole bunch of new data.

  Let's say we have a send window which includes 5, 6 and 7. And 5 is
  lost, but 6 and 7 arrive at the receiver and are acknowledged. The
  sender can not advance the window past five until five is
  acknowledged. So the send window is stalling. The window can stall
  where most of the data in the window has been delivered. It can not
  move past the first unacknowledged piece of data. So it can not
  advance the window past that.

  The receiver also maintains three variables:
  - receive window size
  - the last acceptable segments - this is the last segment it will
    receive and won't drop on the floor. If it receives segment past
    this value, it will assume something is wrong or it's not gonna
    buffer, it will just discard it
  - last segment actually received

  So the receiver maintains this invariant, that the last acceptable
  segment minus that last thing we received must be less than or equal
  to the receive window size. So if we have a "receive window" size
  equal to 5 and a last segment received equal to 3, then its not
  going to accept anything past 4, 5, 6, 7 and 8. So if it receives
  suddenly segment 10, it won't accept it and it will drop it. Now if
  the receive packet is less than this acceptable packet, then it will
  send an acknowledgement.

** Cumulative Acknowledgement
   We send an acknowledgment for not the data we received, but rather
   what is the end of the contiguous data we received, that is
   cumulative. If we acknowledge 3, that means that I have received 3
   and everything before it, not just 3. And so it represents a
   cumulative state of reception across the entire communication.

   So in this example, if a receiver received one, two, three, and
   five and then suddenly receives five, it doesn't acknowledge five,
   it acknowledges three.

   TCP doesn't acknowledge the data its received but rather N plus
   one. So TCP acknowledgements are in terms of bytes, ans so if TCP
   has received up to byte N, its acknowledgement packets will say N
   plus one. So its the first byte of data that's expected. So if we
   are ever looking at a TCP trace or trying to see how the TCP
   protocol works, we should remember this. The acknowledgement value
   in a TCP header isn't the last byte received, the cumulative
   acknowledgment, rather the next byte, the first missing byte.

** Go-back-N
   If we have a receive window size of one, and a sending window size
   that's larger than one. Well in this case we are gonna need sending
   window size plus one sequence number. Well say send window size is
   equal to three, so the sender sends zero, one, and two, and lets
   say those are all acknowledged, and so the receiver acknowledges a
   zero, and acknowledges one, and acknowledges two. Well when it
   acknowledges zero, the senders gonna send three, slide the window
   forward, when it acknowledges one, its gonna send four, and when it
   acknowledges two, it's gonna send five. So lets say that three is
   dropped. The receiver is still going to receive four and five, so
   it can ack two, its going to send ack two, ack two. The sender is
   going to timeout and re-send three. This is called Go-Back-N
   protocol. Because, the receive window size was one, the receiver
   can not buffer four or five. When a single packet is lost in this
   case three, the sender has to go back and has to re-transmit the
   entire send window worth of packets. It has to re-transmit three,
   four and five. In contrast if the receive window size have been
   three then the receiver could have buffered four and five. The
   sender would only have had to re-transmit three, then we get an ack
   five, and it could go on and send six, seven and eight.
** Flow Control
   TCP uses sliding window protocol for flow control. The receiver,
   specifies the flow control window using the window field, this is
   in terms of bytes. So it basically says this is the buffer size
   that I have on the receiver and so the set of packets that I will
   accept. So a TCP receiver will only handle data equal to
   the *acknowledged sequence number plus the window*. So the sender
   is not allowed to send data past ack plus window. That's to make
   sure it doesn't send data which the receiver is not going to
   buffer. And so this is the way for the receiver to essentially set
   what the send window size is.

** Example
   The sequence number space for the packets - from zero up
   to 29. Let's say we have a receive window size equal to two and a
   send window size equal to three. So, communication begins, and the
   sender is going to send zero, one and two. Let's say all three of
   those packets arrive. So the receiver, receives zero, its gonna
   acknowledge zero. It's then going to receive one, acknowledge one,
   receive two, and acknowledge two. When the sender receives ack
   zero, it will advance the window, it will then send three. When it
   hears the acknowledgement for one, it will advance the window and
   send four. When it hears the acknowledgement for two, it will
   advance the window and send five. Now lets say packet three arrived
   successfully and is acknowledged, but packet four is lost in the
   network.

   So now we have this case where ack three has been sent, packet four
   is lost, then packet five arrives at the receiver. Now the receiver
   is going to send another acknowledgement three, again because of
   cumulative acknowledgements, and now sender heard ack three and
   then another ack three, waits times out and re-sends four. So it
   will resend four and lets say four arrives. Now this receiver can
   acknowledge four so it can ack four, but because its received
   window size was two, it actually had five buffered, and so it can
   acknowledge five, and will send ack five.

   So the sliding window flow control algorithm allows an
   unacknowledged, a whole window of unacknowledged packets to be in
   flight. And so this allows, if we can set the window size
   appropriately, it allows the sender to be able to actually fully
   utilize the capacity that the receiver has.

** Window size
   Generally speaking if the two windows are the same size, we need
   basically their sum, RWS plus SWS sequence number. We need sequence
   number spaces at least as big as the sum of the window sizes.

   TCP uses a large sequence number space just for ease of use and to
   really be robust against heavily delayed packets.

* Re-transmission Strategies
  We are maintaining our re-transmission timer for each of these
  packets based on when they were sent. And essentially maintaining
  conservative estimate of, if we haven't received the acknowledgement
  for the packet by this time. Then this means that it is almost
  certain that packet was lost and so we should re-transmit it. And so
  the question is given this set of parameters which are generally
  used for reliable transport, how is the protocol going to behave?
  What is its re-transmission strategy going to look like.

** Go-back-N
   This is a very pessimistic approach, that is because if a single
   packet is lost, then we are going to re-transmit the entire
   outstanding window of packets.

   So with a window size of N if we lose a packet, we are going back N
   transmission, so we transmit all of them.

   So, lets say we have a window of size equal to four, so the sender
   sends packet one, two, three, four and packet two is lost. So here
   are our four transmissions. In response to packet one, the receiver
   is going to send acknowledgement one. But its not going to send
   acknowledgement two. Then in the go back n protocol, what the
   sender is going to do is it's going to re-transmit the entire
   outstanding window into some kind of timing. So window is going to
   include five because in response to this one, it can send five. So
   the transmitter seeing the packet two was lost, its going to assume
   that the entire window is lost and re-transmit the entire
   window. So its conservative, very pessimistic.

** Selective repeat
   This is a very optimistic approach. If we lose a packet, its not
   acknowledged, we will re-transmit that packet and only that packet.
   The receiver accepts out-of-order frames and buffers them. The
   sender individually transmit frames that have timed out.

   So, lets say we have a window of size N=four, we transmit one, two,
   three, four. Packet two is lost, packet one is acknowledged, which
   lets us send five. And the selective repeat protocol, the
   transmitter is going to re-transmit two and we will continue
   execution and transmit six, seven, eight and nine. So re-transmit
   only the packet that were not acknowledged.

   In selective repeat, if actually all of those packets were lost, if
   packet two, three, four and five were all lost, in order to do each
   of this re-transmission involves timers and round trip times. So it
   can be much slower. If there is a burst of losses, a selective
   repeat protocol will be off and slower to recover.

   So there is a trade off between amount of data we send, how quickly
   we send it and then how much of it is wasted versus the speed of
   recovery from significant errors.

** Examples
*** Sender's window=4 and receiver's window=1
    Lets walk through two example transport protocols and their
    configuration and see how they behave, what happens. So in this
    first one, our sender has a window of size N=4, and the receiver
    has a window of size one. So based on this, is the protocol going
    to behave just go back N or selective repeat. So let's walk
    through what happens.

    So the sender is going to send one, two, three, four. And lets say
    packet two is lost so it doesn't arrive. Receiver is going to
    acknowledge one, which will allow sender to send five. But the
    receiver is not going to acknowledge two. Now at some point, its
    retransmission timer is going to fire, and it will retransmit
    two. But the thing is, that because the receiver has a receive
    window size of only one, it has been unable to buffer packets
    three, four and five. And so when it receives packet two, its
    going to ack two. The sender has not received ack for three, its
    going to retransmit three. And the receiver can then acknowledge
    three. The fact that receiver has a window size of only one, its
    going to force the sender to retransmit every single packet in the
    window. So we see that this behaves as go-back-n.

*** Sender's window=n and receiver's window=n
    In this example, sender has a window size of n=4 and the receiver
    has a window size of n=4. In this case protocol be go-back-n or
    selective repeat.

    So the sender is going to send one, two, three and four. Two is
    lost. Ack one result in packet five being sent. At some point
    two's retransmission timer fires so we resend two. Now the
    receiver has been able to buffer these packets because there is a
    window of size n, so it had three packets buffered, so when packet
    two arrives it can then acknowledge five. It might be that the
    sender was little aggressive, maybe it did retransmit three or
    four or something, but the point is that it doesn't have to. So
    for this to operate correctly, say sender just waited for those
    transmission timers or did slow transmissions, etc. that sender is
    going to resend only packet two. Only the outstanding packet that
    was not acknowledged, the rest of the buffered. So we see this
    behaves as selective repeat.

** Design approach
   So when we are implementing transmit protocols, the main thing we
   want to think about is how to handle retransmission. One of the
   important thing is we don't retransmit earlier than we should. Its
   not okay when two has retransmit timer fires, retransmit entire
   window. Because it could be very well that three, four and five
   have been correctly received. By retransmitting early we are very
   aggressively putting additional packets in the network, three, four
   and five could still be in the network, but with retransmission we
   are putting additional copies of them.

   So, in that way we want to be very careful about the number of
   packets we put in there. We have to be careful about the
   retranmission policy.
   
   - one packet was lost ::
     + retransmit entire window with the go-back-n policy.
     + retransmit one packet, wait for one round trip time, and see if
       we get acknowledgement and see where the acknowledgement puts
       and then just do selective repeat.

* Reliable Communications - TCP Header
** TCP header fields
   Standard TCP header is 20 bytes long. So there are five rows of
   four octets each.

   - First two fields in the header are the =source port= and
     =destination port=. Each of these are sixteen bits or two octets.

   - The next two fields are the TCP sequence number. So this denote
     from the source of this packet to its destination, what is the
     sequence number of the data contained in this segment, as well as
     what is the acknowledgement number from that endpoint.

     The =sequence number= denotes the first byte of the data region,
     which follows the segment header. The =acknowledgement number=
     acknowledges the last byte received plus one. And so if we were
     to send the segment 4000 to 4999 and the other side received it,
     it would have send an acknowledgement number of 4500. That is in
     TCP, the ACK is not for the last byte received, but that plus
     one.

     TCP ACK packets are TCP segments that have no data. All they are
     doing is counting the acknowledgement numbers for it. This
     happens when traffic is unidirectional, sending lots of data in
     one direction, but there is no data coming back. If the flow is
     bidirectional then these acknowledgement numbers are just going
     to be incorporated into the data segment as they are being sent.
   
   - The next field is =checksum=, which is computed over the TCP
     pseudo header, which is the - TCP header as well as some of the
     IP header. So, the checksum covers the pseudo header, the TCP
     header, and then the data within the TCP segment. The checksum in
     some way stretches before the packet to the pseudo header filled
     in from the IP header and stretches to the end of the
     segment. Checksum is simple one's compliment.
   
   - The window field is the flow control window. It tells the
     endpoint, how much receive buffer space its sender has. So if we
     say window of size 20,000 that means that there cannot be more
     than 20,000 outstanding unacknowledged bytes in this connection,
     in that direction.

   - Control bits: U, A, P, R, S and F
     + U: Urgent bit - that means this data is particularly urgent. So
       lets get to the application quickly.
     + P: Push bit - push the data to the receiving application.
     + R: Reset bit, which says we need to reset the connection.
     + ACK bit - is set to one, if the acknowledgement number field is
       valid. So ack bit is set to one for every single packet except
       for the first one that initiates the connection. Because when
       we initiate a connection, we don't know what the other side's
       sequence number is, we can not acknowledge anything, so the ack
       bit is not set. The first packet sent does not have =ACK bit=
       set but all other packets in the connection towards the
       termination have the =ACK bit= set.
     + SYN bit - it says this is the starting number please
       synchronize to this number. So when we first open a connection
       we sent a packet with the =ACK bit= not set but with the =SYN
       bit= set and then the sequence number. The other side can then
       respond and say alright I am going to acknowledge that sequence
       number and send you one of mine. In this case both of these
       fields are valid. To which then initial sender will respond and
       say okay I acknowledge your sequence number. Now both know when
       the byte start.
     + FYN bit - it says I have no more data to send.

   - Offset field: This tells where the data begins. The offset field
     is needed because its possible for TCP to have options and we
     don't know from the header necessarily where the options are.

     So if we have options then the offset tells us the size of those
     options and our TCP stack knows to look inside these for options.

** Why sequence number
*** Security concern
    We could always start sequence number at zero for every connection
    for both direction but it turn out to be a real security problem
    doing that. That means people can guess what's our sequence number
    is. They could start intercepting packets.

*** Management concern
    If we have lots of short lived connections, these packets with
    similar sequence numbers can be long lived with the network and we
    won't be able to filter them out.
* Reliable Communications - Connection setup and Teardown
  The high level problem is, if we want to communicate reliably, it
  turns out its very helpful to have state on one or both ends of the
  communication. We can communicate reliably with having something
  stateless on one end or the other, but its much less
  efficient. Having little bit of state is great, it will have much
  better throughput.

  But, if we have this state, there is this problem of, how do we set
  up that state? what is it? So in connection establishment, given the
  states is going to take up RAM in our machine, when can we tear it
  down. When is can we sort of garbage collect this state, and reuse
  it? So for example, the memory structure used for our TCP
  connection, the buffers, also the port numbers that we used. There
  are these problems of connection establishment and teardown.

** Connection Setup
*** 3-way handshake Connection setup
    For connection set up there are four parts of the header that are
    used.
    - Sequence number
    ...
    In the standard 3-way handshake model, we have an active opener and
    a passive opener. The passive opener is sitting, listening, waiting
    for a connection request, such as a web server. The active opener
    is the one who initiates the request, the request to start the
    connection.
    
**** First step
     In the first step, the active opener sends a TCP segment with the
     =SYN bit= set to indicate that its synchronizing the passive side
     to the beginning of its stream. Lets call sequence number to be
     =S(a)=. We do this rather than just say assume zero, for a bunch
     of reasons. Its very helpful to randomize starting sequence
     number for security reasons.

     - It means people can't guess where our stream starts and try to
       insert data.

     - If there happen to be old packets flying around the internet,
       which sometimes happens, we get tremendous delays somewhere. If
       we randomize our starting sequence number, then it becomes very
       unlikely that some random segment or perhaps a corrupted
       segment will overlap our own sequence window.
    
**** Second step
     The passive side responds also with the SYN saying okay, I am
     going to synchronize you, with my starting sequence number
     =S(p)=. But it will also set the =ACK bit= which means that the
     acknowledgement number in the packet is valid, ACK =S(a+1)=. Node
     acknowledges not the last byte received, but rather the first
     byte that has not been received.

**** Third step
     The active side then responds. It does not need to send the SYN
     because its synchronized. So it sends a packet with sequence
     number =S(a+1)= and ACK =S(p+1)=. This initial packet, the
     sequence number is =S(a+1)=, but it tends to be of zero
     length. So if there were a byte in the packet, it would be
     =S(a+1)= but its not, and this is of zero length. This is just a
     simple control packet, and so there is a sequence number from
     which the bytes would start, but there are no bytes.

*** 4-way - simultaneous open connection setup
    TCP also supports another way of opening connections, called
    simultaneous open. In this both the sides are active. Both know
    each other's port numbers. So, the node on the left knows the port
    number that node on the right is issuing a connection request
    from, and the node on the right knows the same for the node on the
    left. And so they are using the correct port numbers.

**** First step
     In simultaneous open both sides send the SYNs at the same
     time. So one on the left sends a SYN - =S sub a=. But, at the
     same time, node on the right sends a SYN - =S sub p=.

**** Second step
     Then node on the left responds and it sends SYN - =S sub a=,
     ACK - =S sub p+1=. Similarly the one on the right side responds
     with =SYN sub p=, ACK - =S sub a+1=. At this point we have now
     established the connection, both sides are synchronized, we know
     the starting sequence numbers that acknowledged that. This
     connection setup takes four messages rather than three.

** Connection Teardown
   Connection teardown uses the =sequence number=, =acknowledgement
   number field= and =FIN bits=. When a sender sends a packet with the
   FIN bit, it means sender has no more data to send, this is the end
   of the stream. This is causing to call close or shut down, in the
   application. But its not until both sides have nothing to send,
   that we actually terminate the connection because it could be that
   one side is done but the other side has more data to send. And so
   its not until both sides have FINed and we've acknowledged those,
   we can tear things down.

   Example: If A and B are communicating, and A closes first and so it
   sends a packet with a =FIN= bit with sequence number =S sub A= and
   acknowledging =S sub B=. B then sends a packet to acknowledge this
   FIN, so =ACK S sub A + one=. Then at some point later, B decides it
   needs to close its side of connection so it sends a FIN, sequence
   number =S sub b=, which then A responds saying, I will acknowledge
   =S sub b + one=. So, FIN like SYN represents the last byte of the
   connection.

   We can have simultaneous close, where they send the =FINs= in
   parallel and the same exchange occurs. 

*** Can A and B forget about closed socket after final message
    Great so now, we have exchanged these messages and acknowledged
    them. Now when can we actually tear down the connection ? When can
    we actually delete the state ? When can we reuse the ports ? This
    turns out to be nontrivial.

**** Problems with closed sockets
     - Final ACK is lost in the network :: We have sent a FIN, then we
          receive a FIN, then we ACK it. We can't immediately teardown
          the connection because, what happens if the ACK is lost ?
          The other side is never going to hear it. The other side
          never going to know whether the connection was, torn down.

     - Same port pair is immediately reused for a new connection :: Another
          issue is it could be that we do a FIN, FIN ACK, and an
          acknowledgement teardown and then the same port pair, the
          same port pair is used immediately for a new connection. We
          want to make sure that we don't, by accident, then corrupt
          the data because the sequence number of spaces overlap.

**** Solution: active closer goes into TIMEWAIT state
     The solution used is the active closer goes into =TIMEWAIT=. Once
     the connection is torn down, we have to wait a little while
     before we can reuse the state. So we keep the socket around for
     two maximum segment lifetimes, which is on the order of say a
     minute or so.

**** Can pose problems with servers
     This approach of two maximum segment lifetimes can pose problems
     with servers. In particular if we have a server and it has tons
     and tons of sockets, which is in this =time wait= state, this can
     slow down the server, when closing first.

     There are tricks around this, we can send the reset and delete
     the socket. We can set =SO_LINGER= socket option to make the
     linger time to be zero.
     
     Another issue is that OS might not let us reuse a port because
     its still in use, =SO_REUSEADDR= option lets us re-bind used port
     numbers.

* Packet Switching
** Circuit Switching
   - Each call has its own private, guaranteed, isolated data rate
     from end-to-end.

   - A call has three phases:

     1. Establish circuit from end-to-end ("dialing") :: First we pick
          up the handset and dial the number and dialing the number is
          saying where do we want to be connected to. This creates a
          dedicated circuit from one end to another. So that dedicated
          circuit is gonna go through all the circuits along the
          way. And the system has told each of the circuits to connect
          an incoming wire to an outgoing wire. So each switch is
          going to maintain the state to map the incoming circuit to
          the correct outgoing circuit.
	  
     2. Communicate :: In the second phase we talk, in a digital phone
                       system our voice is sampled and digitized at
                       the first switch. Then it is sent over
                       dedicated circuit as a typically 64 kilobits
                       channel for voice.

     3. Close circuit ("tear down") :: The circuit is to be removed
          and any state in the switches along the path has to be
          removed as well.

   In practise between the switches there are trunk lines which are
   really, really fast. They have a very high data rate. Many thousand
   phone call can share the same trunk line between cities.

   The key thing to remember is that every phone call has its own
   dedicated 64kb/s circuit that it doesn't have to share with anyone
   else. Each call has its own private guaranteed isolated data rate
   from end to end.

   Originally, a circuit was an end-to-end physical wire. Nowadays, a
   circuit is like a virtual private wire. Its gonna share that wire
   with others but it has its own dedicated circuit within that wire.

** Problems with circuit switching
   When we are thinking about using circuit switching for the Internet
   or any computer communications, there are few shortcomings that we
   need to consider.

   So we are gonna go through three main problems:

*** Inefficient
    Communication between computers tend to be very bursty. We tend to
    send data in bursts of may be a few seconds, a few packets, may be
    even a few minutes, depending on the application we are
    running. For example if I am typing over an SSH connection then I
    am gonna have characters that I am gonna send every now and
    again. Sometimes a flurry of characters as I type a word and then
    long periods of silence in between. Or if I am reading a sequence
    of web pages, I might have a burst as I fill up one page, a burst
    as I fill up the figures to populate that page, and then a period
    of pause while I read those web pages. So it tends to be very
    bursty. Because during those times of silence, when I am not doing
    anything, there is no activity. There is a dedicated circuit which
    nobody else can use. So it is very inefficient, very inefficient
    use of the capacity of the network as a whole.

*** Fixed rate
    Computers tend to have very diverse applications that need very
    different rates. So a web server might be streaming video at say
    one, five or even six megabits per second. But when we compare
    this with us typing one character every second, there is a huge
    difference in the rates that network needs to support.

    So if we pick a fixed rate circuit for the video and then we use
    it for typing, it will be barely used. Or vice versa then we
    wouldn't be even able to stream the video. So a fixed rate circuit
    really is not much use at all.

*** State maintenance
    We need to maintain some state for every phone call that's going
    on. Every time we establish a call, we need to setup the circuit
    mapping from the ingress to the egress of every switch along the
    way. If a circuit fails, or a switch fails, or a link fails, we
    need to go in and change all of that state in order to reroute the
    calls. So we have to manage it, and at the end we need to remember
    to take the state out. If anything fails then we may find that the
    state becomes inconsistent, and then we have a problem. So state
    management in circuit switching is considered a problem. And if we
    had thousands or hundreds of thousands of communications going on
    at the same time, that just lot of work that has to be done.

** Packet Switching
   In packet switching there is no dedicated circuit to carry our
   data. Instead we just send when we are ready at any time, we want
   to send a block of data by adding a header to it that's what we
   call a packet. A header contains the address of where the packet is
   going just like an envelope tells the post office where to send the
   letter.

   Packet switched network consists of end hosts, the links, and
   packet switches. When we send the packet its routed one hope at a
   time from the source, all the way through to the destination. If we
   look at the packet it has the data in it and it also has the
   address, where its going to.

   So when we send the packet its gonna be routed hop-by-hop from the
   source to the destination. Each packet switch along the way is
   gonna look up the address in a forwarding table. So it keeps a
   local forwarding table at all of the packet switches. S1 will look
   in the forwarding table, and forward it to S2, and then eventually
   to the correct destination.

   In internet there are lots of different types of packet
   switch. Some of them are called routers because they deal with
   internet addresses. There is also called ethernet switches.

   There will be millions and millions of packets flowing in all sorts
   of direction at the same time. So these packet switches have lots
   of work to do.

*** Packet switch features
    - Packet switches have forwarding table to tell where the packet
      goes next.
  
    - They also have *buffers*. If packets arrive at the same time and
      lets say they are arriving at the full link rate, of outgoing
      link, then the packet switch has to hold one while it sends the
      other, it can't send them both at the same time, so its gonna
      send one at a time. And because it might have many incoming
      links, the packet switch has to have a buffer to hold perhaps
      many packets. These packets buffers can be quite large in
      practise. Buffers hold packets, when two normal packets arrive
      at the same time.

      Particularly during congestion, when there are lots of lots
      packets coming in at all the input links, all trying to get to
      the same output, it may actually have quite big buffers to hold
      packets during those times of congestion, to prevent us having
      to drop any packets.

      Packets are routed individually by looking up the address in the
      router's local forwarding table.

    - Routers does not maintain per communication state. It just
      maintains the forwarding table.

*** Why packet switching
    - Packet switching allows us to use expensive links
      efficiently. It allows to share bursty flows at the same
      time. Because at any instant packet can use the entire link, but
      it can be immediately followed by another packet using the
      entire link belonging to a different communication.

    - Packet switching allows more resilient networks. Networks that
      are resilient to failure of links and routers. And the reason
      for this is, that because each packet is individually routed
      along the path, if something happens if something breaks, a link
      breaks or a router breaks. Then we can, because we have no state
      in all of the switches for this particular flow, we can simply
      send the packet on a different path. We can send it over a
      different link, to a different router, and it will find its way
      eventually.

    - Internet was originally designed as an interconnection of the
      existing networks. And at that time, pretty much all widely used
      communication networks were packet switched. So if internet was
      to connect all of those existing networks, then it too needed to
      be packet switched as well.

* End to End delay and queuing delay
** Propagation Delay (t_l)
   Propagation delay is the time it takes a single bit to travel over
   a link at propagation speed =c=. Propagation delay is determined by
   how long the link is. Speed of propagation is very close to the
   speed of light.
   ...
   Propagation delay doesn't depend on the data rate of the
   link. Doesn't matter if the link is running at 1kb/s or 10gb/s. The
   propagation delay is just the function of speed of propagation of
   each bit and the length of the cable.

** Packetization Delay (t_p)
   This the time from when the first bit of the packet is put onto the
   link until the last bit is put onto the link.

   The time it takes to put all of the bits onto the link is gonna be
   function of the number of bits we are putting on to the link, and
   the distance between them all. The number of bits per second that
   we can put onto the link. So essentially the data rate of a link is
   determined by how close together we can pack the bits.

   If for example, a link runs at one gigabit per second, we can put
   ...
   Where =p= is length of the packet in terms of number of bits, and
   =r= is rate at which we put bits on to the link.

   - Example1: A 64byte packet takes 5.12 micro seconds to be
     transmitted onto a 100Mb/s link.
     #+BEGIN_EXAMPLE
     p/r = (64*8)/(100*1000000) = 5.12 micro seconds
     #+END_EXAMPLE

   - Example2: A 1kbit packet takes 1.024 seconds in order to transmit
     onto a 1kb/s link. One kilobit per second means 1000 bits per
     second whereas one kilobit in the packet is 1,024 bits.
     #+BEGIN_EXAMPLE
     p/r = (1*1024)/(1000) = 1.024 seconds
     #+END_EXAMPLE
   
** Queuing Delay Q(t)
   Packets share the links with packets from other users. When several
   packets show up at the same time wanting to go on the same link,
   then some of the packets have to wait in the router's =queue=.

   Some people call this queue =packet buffers=, but in general these
   are first come first serve queues, in which the packets are gonna
   depart in the same order they arrive.

   The =packet buffer= helps prevent us from having to drop any
   packets. The bigger the buffer is the less likely we are to have to
   drop a packet, that wants to travel across the link. These packet
   buffers are going to be in all of the switches. Every packet switch
   has buffers, and they are fundamental to packet switching. If we
   did not have packet buffers, then we would lose a packet every time
   two packets showed up at the same time wanting to travel across the
   same link.

   If our packet arrives and queue has some packets in it, then its
   going to delay the time that it can be forwarded on to the next
   link. Because it will have to wait for the packets that are ahead
   of it to leave first before our packet gets to go. This delay will
   vary over time and it will depend on how many other packets are
   showing up from other users. So, if there are three packets ahead
   of us, we will have to wait for three packetization delays before
   its our turn to go.
...
** End to End Delay
   How long will it take for a packet of length =p= to travel from A
   to B, from when the =1st= bit is sent, until the =last= bit
   arrives.

   End to end delay for the first link would be sum of its
   packetization delay, propagation delay and queuing delay.

   #+BEGIN_EXAMPLE
   end-to-end-delay-for-first-link = p/r + l/c + Q(t)
   #+END_EXAMPLE

   Internet routers are what we call store and forward devices. That
   means switch =S1= is gonna wait until the whole packet arrives,
   until it looks up the address and decides where to send it next. So
   the end to end delay from A to B would be sum of individual end to
   end delay.

   #+BEGIN_EXAMPLE
   end-to-end-delay = sum(p/r_i + l_i/c + Q_i(t))
   #+END_EXAMPLE

   Queuing delay is non deterministic, whereas packetization delay and
   propagation delay are deterministic. Queuing delay depends on the
   traffic sent by other users in the network. So end to end delay is
   unpredictable.

   In summary end to end delay depends on:
   - Propagation delay (fixed)
   - Packetization delay (fixed)
   - Queuing delay (variable)

** Sample problems
   We wish to send a message of size 150,000 bytes over the
   network. There are four hops each of length 20km and running at 100
   Mb/s. However, before sending we split the message into 1500 byte
   packets. What is the end to end delay of the message? Use speed of
   light in copper = c = 2 * 10^8 m/s, and round the answer to nearest
   integer millisecond.

   #+BEGIN_EXAMPLE
   packetization delay = 0.12
   propagation delay = 0.1
   for four hops = 4*(0.12 + 0.1) = 0.88 ms
   
   rest of the message = 150000 - 1500 = 149500
   propagation delay = (149500 * 8)/(100 * 10^6) = 11.96 ms

   total = 11.96 + 0.88 = 12.84 ~ 13 ms
   #+END_EXAMPLE
* COMMENT TODO
  1. join emacs group
  ...
  22. How if active closer has closed the connection, it can
      acknowledges the received data.
  23. If the IP checksum is wrong, what will happen?
  24. Are routers multi threaded, in the sense do they process
      multiple packets at time, or single packet.
  25. Can we add more intelligence into TCP headers? Will intermediate
      devices be able to adapt the change.
  26. What limits the data rate of a link.
* COMMENT References
  -
  ...
  - RFC-1958
  - http://lxr.free-electrons.com/source/net/ipv4/tcp.c (tcp
    implementation)
  - http://locklessinc.com/articles/tcp_checksum/ (checksum)
