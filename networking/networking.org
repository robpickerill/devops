...
*** Acknowledgement
    When a TCP layer receives a packet, it sends an acknowledgement to
    the sender, to let the sender know the data arrived correctly.
...
*** Sequence number
    Sequence number detects missing data. Every segment header carries
    sequence number field at the first byte. Suppose two hosts agree
    on the starting sequence number to be 1000. Then the first segment
    will have sequence number of 1000. If the segment carries 500
    bytes of data, then the next segment will carry sequence number
    of 1500. If a segment gets lost, then the sequence number will be
    incorrect and TCP layer will knows some data is missing. It could
    be that the segment will arrive later - perhaps taking a longer
    route, or it could have gone missing, in that case sender has to
    resend the data.
*** Flow control
    TCP controls the flow of data from sender to the
    receiver. Receiver keeps telling the sender if it can keep sending
    the data. If at any point receiver is full it tells, sender to
    stop sending more data. This prevents overwhelming receiver with
    data, that it could not handle.
* COMMENT TODO
  1. join emacs group
