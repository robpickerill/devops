...
* End to End Principle
  The function in question can be completely and correctly implemented
  only with the knowledge and help of the application at the end
  ...
  responsible for making sure it does. Nobody else has the information
  necessary to do this correctly.
...
** Wireless - performance enhancement
   According to end to end argument, correctness features should be
   implemented at the end points. The network can include an
   ...
   TCP doesn't work well with low reliability. To improve reliability,
   wireless link layer re-transmits the packets at the link layer. For
   example when our laptop sends data to access point, the access
   point after receiving the packet immediately -- just a few
   microseconds later -- sends a link layer acknowledgement back to
   the laptop. If the laptop does not receive a link layer
   acknowledgement it re-transmits. It does this several times. These
   link layer acknowledgment can boost the poor link, with 80%
