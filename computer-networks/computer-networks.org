...
* Reliable Communications - TCP Headers
  Standard TCP headers is 20 bytes long. So there are five rows of
  four octets each. 
  ...
  - The next two fields are the TCP sequence number. So this denote
    ...
    acknowledges the last byte received plus one. And so if I were to
    ...
    be incorporated onto the data segment as they are being sent.
