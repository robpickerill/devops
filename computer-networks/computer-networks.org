...
** Propogation Delay (t_l)
   The time it takes a single bit to travel over a link at propogation
   speed =c=. Propogation delay is determined by how long the link
   is. Speed of propogation is very close to the speed of light.

   #+BEGIN_EXAMPLE
   t_l = l/c
   #+END_EXAMPLE

   Propogation delay doesn't depend on the data rate of the
   link. Doesn't matter if the link is running at 1kb/s or 10gb/s. The
   propogation delay is just the function of speed of propogation of
   each bit and the length of the cable.

** Packetization Delay (t_p)
   The time from when the first to the last bit of a packet is
   transmitted.
   ...
   - Example2: A 1kbit packet takes 1.024 seconds to be transmitted
     onto a 1kb/s link. 1kbit packet actually means 1024 bits packet
     and 1kb/s means 1000 bits per second.
     #+BEGIN_EXAMPLE
...
** End to End Delay
   ...
   packetization delay and propogation delay. This is because bits are
   travelling one behind another and it takes time equal to
   packetization delay to put last bit on the network.
   ...
   We also need to incorporate congestion delay also in our overall
   end to end delay. =Q(t)= is the queuing delay of a packet in the
   queue due to congestion. Some people call queue as packet buffers
   but generally these are first come first serve queue.
