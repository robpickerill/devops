...
** Packet Switching
   ...
   - Buffers :: If packets arrive at the same time and lets say they
                are arriving at the full line rate, of outgoing link,
                then the packet has to hold one while it sends the
                other, it can't send them both at the same time. So
                its gonna send one at a time. And because it might
                have many incoming links, the packet switch has to
                have a buffer to hold perhaps many packets. These
                packets buffers can be quite large in
                practise. Buffers hold packets, when two normal
                packets arrive at the same time.

		Particularly during congestion, when there are lots of
                lots packets coming in at all the input links, all
                trying to get to same output, it may actually have
                quite big buffers to hold packets during those times
                of congestion.
