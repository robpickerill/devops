...
*** Second step
    ...
    =S(b)=. But it will also set the =ACK bit= which means that the
...
*** Third step
    ...
    number =S(a+1)= and ACK =S(b+1)=. This packet the sequence number
    is =S(a+1)=, but it tends to be of zero length.
