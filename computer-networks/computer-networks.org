...
** Connection Teardown
   ...
   of the stream, this is the end of the stream. This is causing to
   call close or shut down, in the application. But its not until both
   sides have nothing to send that we actually terminate the
   connection because it could be that one side is done but the other
   side has more data to send. And so its not until both sides have
   FINed and we've acknowledged those, we can tear things down.
