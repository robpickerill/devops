...
*** Go-back-N
    ...
    seven and eight.
*** something
    The receiver, specifies the flow control window using the window
    field, this in terms of bytes. So it basically says this is the
    buffer size that I have on the receiver and so the set of packets
    that I will accept. So a TCP receiver will only handle data equal
    to the acknowledged sequence number plus the window. So the sender
