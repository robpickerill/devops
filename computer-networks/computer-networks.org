...
**** Failure
     ...
     protocol, it comes up in flow control, which is how do we detect
     duplicates. How do we know when acknowledgements are from
     re-transmissions or duplicated copies of packets versus new data.

     So, in the case of stop and wait, we can solve the problem with a
     one bit counter. And so the idea is that use this one bit counter
     on all data and acknowledgement packets.
