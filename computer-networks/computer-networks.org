...
*** First step
    ...
    of reasons. Its very helpful to randomize starting sequence number
    ...
    - If there happen to be old packets flying around the internet,
      ...
      unlikely that some random segment or perhaps a corrupted segment
      will overlap our own sequence window.
...
*** Second step
    ...
    acknowledges not the last byte received, but rather the first byte
...
*** Third step
    ...
    sequence number is =S(a+1)=, but it tends to be of zero length. So
    if there were a byte in the packet, it would be =S(a+1)= but its
    not, and this is of zero length. This is just a simple control
    packet, and so there is a sequence number from which the bytes
    would start, but there are no bytes.
...
** Simultaneous open connection setup
   ...
   number that node on the right is issuing a connection request from,
   the node on the right knows the same for the node on the left. And
   so they are using the correct port numbers.

*** First step
    In simultaneous open both sides send the SYNs at the same time. So
    one on the left sends a SYN - =S sub a=. But, at the same time,
    node on the right sends a SYN - =S sub p=.

*** Second step
    Then node on the left responds and it sends SYN - =S sub a=, ACK -
    =S sub p+1=. Similarly the one on the right side responds with
    =SYN sub p=, ACK - =S sub a+1=. At this point we have now
