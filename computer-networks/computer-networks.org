...
** Go-back-N
   If a single packet is lost, then we are going to re-transmit
   outstanding window of packets. This is a pessimistic approach.

** Selective repeat
   One loss will lead to only that packet re-transmitting. This is
   optimistic approach.
