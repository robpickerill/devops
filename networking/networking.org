...
** Link layer
   ...
   hop-by-hop over each link in turn. Ethernet and wifi are two
   examples of link layer. Data is delivered in packets. Packet is a
   self contained unit which contains data to be delivered and headers
   such as - where it came from, where it is going and so on.
...
** Network layer
   ...
   Network layer uses IP protocol. Which provides following features:
   1. Best effort attempt to deliver datagrams but no promises.
