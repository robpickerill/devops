...
** Operations that do not require root privileges - SUID
   ...
   Example of a file, with suid set:
   #+BEGIN_EXAMPLE
...
** Sudoers file
   ...
   We can edit the sudoers file using =visudo= command. This ensures
   that only one person is editing the file at a time, has the proper
   permissions, refuses to write out the file and exit if there is an
...
** Passwords
   The system verifies authenticity and identify using user
...
** Password Encryption
   ...
   called *SHA-12* (Secure Hashing Algorithm 512 bits). The SHA-12
   algorithm is widely used for security applications and
   protocols. These security applications and protocols include TLS,
   SSL, PHP, SSH, S/MIME and IPSec. *SHA-12* is one of the most tested
   hashing algorithms.
...
** Labs
   sudoers file must be edited carefully. If there are any mistakes in
   the file, then you will not be able to execute the sudo
   command. Hence the best way to edit sudoers file is to use =visudo=
   command, for example:
   #+BEGIN_EXAMPLE
   sudo visudo -f /etc/sudoers.d/user1
   #+END_EXAMPLE

  This way if there are any errors in the file, visudo will prompt to
  correct it, and prevent from landing in some difficult situation.

  If there are any syntax errors in the sudoers file then you will not
  be able to execute root privileged command. Then you have to use
  ...
  =pkexec= is a similar command to =sudo=. It allows user to execute a
...
* COMMENT TODO
  ...
  - Revise security principles chapter.
