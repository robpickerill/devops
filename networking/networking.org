...
** Router
   A router works according to its routing table. Routing table
   contains ip addresses patterns to link pair. When a packet arrives
   at the router, it matches the destination address against the IP
   patterns. It finds the best match, and decides where to forward
   the incoming packets.

   Router itself has an ip address. So router might forward the
   packet to its software, destined to them. This happens when we
   connect to router over TCP.

   For example router may also contain default route, which matches
   every packets and forwarded to a particular link.
