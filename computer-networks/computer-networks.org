...
** Sliding Window
   ...
   the *size of its sending window*, the *last acknowledgement it
   received from the receiver* and the *last segment it sent*. The
   sender job is to maintain this invariant, that the last segment
...
*** Flow Control
    ...
    specifies the flow control window using the window field, this in
    terms of bytes. So it basically says this is the buffer size that
    I have on the receiver and so the set of packets that I will
